...
void createBuffer(key_t key, int shmsize, const char * type, int size) {
    ...
    if (strcmp(type, "async") == 0) {
        ...
        // new for MacOS
        char semName1[32], semName2[32], semName3[32];
        snprintf(semName1, sizeof(semName1), "/mutex-%d", key);
        snprintf(semName2, sizeof(semName2), "/slotsEmpty-%d", key);
        snprintf(semName3, sizeof(semName3), "/slotsFull-%d", key);

        buf->mutex = sem_open(semName1, O_CREAT, 0644, 1);
        buf->slotsEmptyMutex = sem_open(semName2, O_CREAT, 0644, size);
        buf->slotsFullMutex = sem_open(semName3, O_CREAT, 0644, 0);
    ...
    /*
        ...
        if (sem_open(buf->mutex, 1, 1) == -1 ||
            sem_open(buf->slotsEmptyMutex, 1, buf->size) == -1 ||
            sem_open(buf->slotsFullMutex, 1, 0) == -1) {
            perror("sem_open error");
            exit(1);
        }*/
    } else {
...
// close and unlink semaphores when you're done:
void cleanupSemaphores(key_t key) {
    char semName1[20];
    char semName2[20];
    char semName3[20];

    sprintf(semName1, "/sem-mutex-%d", key);
    sprintf(semName2, "/sem-empty-%d", key);
    sprintf(semName3, "/sem-full-%d", key);
